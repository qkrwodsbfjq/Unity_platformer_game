...
{
	NodeType<ItemType>* first_ptr = new NodeType<ItemType>;
	NodeType<ItemType>* current_ptr = first_ptr;
	current_ptr->next = listData;

	while (current_ptr->next != NULL && current_ptr->next->info <= item) {
		if (current_ptr->next->info == item) {
			NodeType<ItemType>* temp_ptr;
